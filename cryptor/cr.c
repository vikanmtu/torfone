...
short cr_process(unsigned char* data, short inlen)
{
 ...
 switch(type) //check for packet type
 {
  ...
  case EVENT_WAKE:{len=cr_event_wake(data); break;} //busy event event from TR
  ...
  case CR_NAME:{len=cr_st_N(data); break;} //get incoming name finded by key
...
short cr_st_B(unsigned char* data)
{
  ...
  if(!data[2]) //iddle mode: setup outgoing call
  {
   ...
   if(data[3]) cc->self=1; //set self flag for outgoing call to myself (test)
...
//new phisical incoming connecting
short cr_event_wake(unsigned char* data)
{
 if(data[1]) return 0; //busy
 data[0]=UI_WAKE;
 return EVENT_LEN;
}
...
short cr_event_accept(unsigned char* data)
{
 ...
 if(!cc->self) au_set(AUDIO_CONNECT); //play connect tone during IKE
...
//storage returns name of incoming key in our book
short cr_st_N(unsigned char* data)
{
  unsigned char* p=data+4;
  short i;

  volatile unsigned char is_name;
  volatile unsigned char c;
  memcpy(cc->N, p, 32); //copy incoming name

  //check name is empty (not found) and replase out outhenticator by random
  //in constant time

  //not authenticate received contacts before renaming
  c=p[0]^'+'; //0 if first char is +
  c=!c; //1 if first char is + else 0
  c--;  //0 if first char is + else FF
  p[0]&=c; //0 if first char is + else unchanged
  is_name=0xFF^((!p[0])-1); //set flag is 0 if contact finded else 0xFF

  ike_rand(p, 16); //generate random
  for(i=0;i<16;i++) cc->x[i]^=(p[i]&is_name); //constant time mask our authenticator if contact not in our book

  memcpy(p, cc->x, 32);  //output Mb+My to remote
  data[0]=TR_Q;
  return IKE_LEN;
}
