...
short ike_10(unsigned char* data)
{
 ...
 memcpy(p, cc->B, 32); //output their key to storage
 data[0]=ST_GETNAME;  //get their name  by their key form our book
...
short ike_14(unsigned char* data)
{
 ...
 memcpy(p, cc->N, 32); //output their name
 data[0]=UI_NAME;  //to UI
 data[1]=0xFF&cc->cnt_out; //set au result  00 or FE or FH
 data[2]=cc->sas>>8;
 data[3]=0xFF&cc->sas;  //output SAS
 //clear data after IKE
