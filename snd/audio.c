...
//internal procedures
void setmklevel(short* spc);
void setsplevel(short* spc);
void getsplevel(const short* spc);
void getmklevel(const short* spc);
...
void getsplevel(const short* spc)
{
 ...
 au.splevel -= (au.splevel>>SP_ECHOSMOOTH);  //smooth mike level
 au.splevel += e;             //adjust mike level
}


//------------------------------------------------------------------------------
//check level of recording speech for smoothly adjusting playing volume value
//------------------------------------------------------------------------------
void getmklevel(const short* spc)
{
 int i;
 ...
 for(i=0;i<AMRFRAMELEN;i++) e+=abs(spc[i]); //summ of amplitudes
 e>>=7; // /=128: mean amplitude
 au.mklevel -= (au.mklevel>>MK_ECHOSMOOTH);  //smooth mike level
 au.mklevel += e;             //adjust mike level
}
...
void setmklevel(short* spc)
{
 ...
 au.echo=SP_LEVEL;
 i=au.splevel>>SP_ECHOSMOOTH; //0-32767  average amplitude
 if(i>SP_MAXLEVEL) i=SP_MAXLEVEL; //up trashold
 if(i>SP_MINLEVEL) //down trashold
 {
  i=256-(au.echo*(i-SP_MINLEVEL))/(SP_MAXLEVEL-SP_MINLEVEL); //value 0-255 for decrese mike sensitivity
  if(i>256) i=256;
  if(i<0) i=0;
...
//------------------------------------------------------------------------------
//decrease speaker volume depends current sensitivity value
//------------------------------------------------------------------------------
void setsplevel(short* spc)
{
 ...
 au.volume=MK_LEVEL;
 i=au.mklevel>>MK_ECHOSMOOTH; //0-32767  average amplitude
 if(i>MK_MAXLEVEL) i=MK_MAXLEVEL; //up trashold
 if(i>MK_MINLEVEL) //down trashold
 {
  i=256-(au.volume*(i-MK_MINLEVEL))/(MK_MAXLEVEL-MK_MINLEVEL); //value 0-255 for decrese mike sensitivity
  if(i>256) i=256;
  if(i<0) i=0;
  volume(spc, (unsigned short)i); //apply
 }
...
int au_create(char* audio_in, char* audio_out)
{
  ...
  au.volume=0; //volume supression level
...
int au_play(unsigned char* pkt)
{
 ...
 i=decrypt(pkt);  //decrypt voice packet
 if(!i) pkt[0]|=0x40; //set silency flag on decryption was fail
 ...
 ////////if(au.volume) setsplevel(au.inbuf); //supress speacker in mike activity
 ...
 if((playdelay<1500)||(playdelay>2000)) //check the low-level buffer have space
 {
  //play speech frame
...
//------------------------------------------------------------------------------
 ...
 int au_grab(unsigned char* pkt)
 {
  ...
  /////////getmklevel(au.inbuf); //measure mike level for supress speaker
  /////////if(au.echo) setmklevel(au.inbuf);  //dinamically decrease mike sensitivity for echo supressing
  ...
  if(au.amrenstate)  //check amr codec is initialized
  {
   ...
   i=encrypt(pkt); //encrypt packet
   if(!i) //if encryption was fail
   {
    memset(pkt, 0, 16); //clear voice data
    pkt[0]=0x40;  //set silency flag
   }
  }
