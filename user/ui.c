...
void ud_addnew(unsigned char* data)
{
 ...
 {
  memcpy(gui.list, p, sizeof(gui.list));
  ui_setcommand(GUI_LIST_ADD); //add contact to List
  ui_setcommand(GUI_BTNNEW_G);  //notify creating OK
 }
 else //book fail
 {
  if(data[1]==STORAGE_NOSPACE) strcpy(gui.info, "No space!");
  else if(data[1]==STORAGE_EXIST) strcpy(gui.info, "Already exist!");
  else strcpy(gui.info, "Storage error!");
  ui_setcommand(GUI_INFO_TEXT);
  ui_setcommand(GUI_INFO_R);
 }
}
...
void ud_setadr(unsigned char* data)
{
 ...
 else if(data[1]!=STORAGE_OK) 
 {
  ...
  if(data[1]==STORAGE_UNINIT) strcpy(gui.info, "Storage not ready!");
  else if(data[1]==STORAGE_NONAME) strcpy(gui.info, "Empty name!");
...
//----------------------DURING CALL: send contact to remote-----------------------------
 ...
 else if(ui.iscall) //in call on IKE: make doubling call
 {
  ...
  if((ui.iscall!=3)||(gui.name[0]=='+')) p[16]=0; //only if current call is incoming from Onion
 ...
 else //not in call: update adress field
 {
  ...
  ui_setcommand(GUI_ADDR_W); //show adress
...
//------------------DURING CALL: rerquest of allow key saving--------------
 ...
 if(ui.istalk) //in call: request for allow to add new key
 {
  if(!ui.iskey)
  {
   strcpy(gui.info, "Key receiving deny!");
   ui_setcommand(GUI_INFO_TEXT);
   ui_setcommand(GUI_INFO_B);
   ui_setcommand(GUI_BTNKEY_B); //key saving denied
  }
  else ui_setcommand(GUI_BTNKEY_W);  //key saving allowed
...
void ud_setupd(unsigned char* data)
{
 ...
 gui.name[0]=0;
 ui_setcommand(GUI_NAME_TEXT);
 ui_setcommand(GUI_NAME_W); //set name field not yellow
 gui.addr[0]=0;
 ui_setcommand(GUI_ADDR_TEXT);
 ui_setcommand(GUI_ADDR_W); //set address field not yellow
 ...
 {    //edit list: remove old contact and add new
  ...
  ui_setcommand(GUI_LIST_DEL); //delete old name from list
  memcpy(gui.list, newname, sizeof(gui.list));
  ui_setcommand(GUI_LIST_ADD); //add new name to list
...
void ud_setdel(unsigned char* data)
{
 ...
 else
 {
  ...
  ui_setcommand(GUI_INFO_TEXT);
  ui_setcommand(GUI_INFO_W);  //set result black
 }
...
//========================================================================
//   TR reports: new phisical connecting, wake lock
//========================================================================
void ud_setwake(unsigned char* data)
{
 cn_cnt=SLEEP_CMD; //restart sleep timer
 ui_setcommand(GUI_WAKE);
}
...
if(au)
 {
  if(gui.name[0])
  {
   ui_setcommand(GUI_NAME_G);  //name icon green
   ui_setcommand(GUI_AU_B); //contact in our book
  }
  else ui_setcommand(GUI_AU_R); //contact not in our book but know our name
 }
 ...
 //and request adreess of incoming contact
  if( au && (ui.iscall>1) && gui.name[0]) //au ok and this is incoming call
 {
...
void ud_setcall(unsigned char* data)
{
 ...
 //(while you trust Tor)
 if(strstr(ui.addr, (char*)".onion")) ui_setcommand(GUI_ADDR_G);
 ...
 if(au)
 {
  ui_setcommand(GUI_NAME_G);
  if(strstr(ui.addr, (char*)".onion")) ui_setcommand(GUI_AU_G);
  else ui_setcommand(GUI_AU_B);
 }
...
void ud_setsec(unsigned char* data)
{
 if(!ui.istalk) return;
 //show result
 if(data[1]) ui_setcommand(GUI_BTNSPK_R); //speke fail
 else ui_setcommand(GUI_BTNSPK_G); //speke OK
 //check need answer
...
void ud_getkey(unsigned char* data)
{
 ...
 else
 {
  ...
  else if(data[1]==STORAGE_NONAME) strcpy(gui.info, "Name already exist!");
  else if(data[1]==STORAGE_WRFAIL) strcpy(gui.info, "ST+ write error!");
  ui_setcommand(GUI_INFO_TEXT);
  ui_setcommand(GUI_INFO_R);
...
void ud_setdbl(unsigned char* data)
{
 ...
 if(!data[1])
 {
  ui_setcommand(GUI_ADDR_G); //set their onion adress valid for connect outgoing doble
  ui_setcommand(GUI_AU_G);  //address authentification OK
 }
 //set doubling state
 if(data[1]==3) ui_setcommand(GUI_BTNTRM_B);//doubling disabled: only incoming work now icon=Blue
 else if(data[1]==4) ui_setcommand(GUI_BTNTRM_R);//doubling disabled: only outgoing work now: icon=Black
 else ui_setcommand(GUI_BTNTRM_G);//0 or 2: new outgoing or Tor incoming ready: doubling enabled now: icon=Green
}
...
void ud_setdir(unsigned char* data)
{
 ...
 if(data[1]==2) ui_setcommand(GUI_BTNDIR_G);//double active
 else if(data[1]==1) ui_setcommand(GUI_BTNDIR_W);//double denied
 else ui_setcommand(GUI_BTNDIR_B); //double remote request
}
...
void ud_setrst(unsigned char* data)
{
 ...
 ui_setcommand(GUI_STATE_IDDLE); //reset GUI to iddle state
 gui.note[0]=0;
...
void ui_dochange(char* name, char* addr)  //change current contact
{
  ...
  ui_setcommand(GUI_BTNCHG_W); //deny changes (for result)
...
void ui_docall(char* name, char* addr) //outgoing call
{
 ...
 //initiate outgoing call
  ...
  ui_setcommand(GUI_STATE_TERM); //set GUI state: init outgoing call
...
{
 if(!ui.istalk) return;
 if(on) ui_setcommand(GUI_BTNDIR_R);
 else ui_setcommand(GUI_BTNDIR_W);
 memset(ui.out, 0, 36);
 ui.out[0]=TR_DIR;
...
{  //called from GUI C++ code
 ...
 if(len)  //check data packet was received
 {
  ...
  switch(type)  //process packet by it's type
  {
   ...
   //call
   case(UI_WAKE):{ud_setwake(ui.buf); break;}  //notify new incoming phisical connecting
