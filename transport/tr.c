...
//**************************************************************************
#define USE_TELNET 7770   //use telnet control for upper ports
...
//------------flag==0-------------------------------------  
  ...
  if(!tr.obf)
  {
   ...
   tr.time_for_ping=tr.time_now+PING_INTERVAL;  //180 sec
   tr.soc_event=EVENT_WAKE;  //wake system
   return 3;
  }
...
//SET ADRESS OF USED STUN SERVER
  ...
  if((tr.listen_port > USE_TELNET)&(tr.listen_port < (USE_TELNET+10))) fl_init(tr.listen_port+1); //run EKG telnet server telten on tcpport+1
